...
resource "azurerm_resource_group" "resource_group" {
  name     = "azratasks-${lower(local.environment)}-rg"
  location = var.location

  tags = local.tags
}
resource "azuread_group" "admins_group" {
  display_name     = "AzraTasks-${local.environment}-admins"
  security_enabled = true
}

resource "azurerm_user_assigned_identity" "app_identity" {
  name                = "azratasks-${lower(local.environment)}-mi"
...
module "container_app_environment" {
  ...
  container_app_environment_name = "azratasks-${lower(local.environment)}-cae"
...
module "backend_container_app" {
  ...
  name                            = "azratasks-${lower(local.environment)}-backend"
...
module "static_web_app" {
  ...
  name           = "azratasks-${lower(local.environment)}-swa"
  resource_group = azurerm_resource_group.resource_group
  sku = {
    tier = "Free"
    size = "Free"
...
module "sql" {
  ...
  server_name   = "azratasks-${lower(local.environment)}-sqlserver"
  database_name = "azratasks-${lower(local.environment)}-db"
