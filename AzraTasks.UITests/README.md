...
2. Ensure the AzraTasks is running at the configured base URL
...
```bash
dotnet run --project AzraTasks.AppHost
```
...
- name: Install Playwright Browsers
  run: pwsh AzraTasks.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- name: Run UI Tests
  env:
    TEST_BASE_URL: https://staging.example.com
  run: dotnet test AzraTasks.UITests/AzraTasks.UITests.csproj
```
...
- task: PowerShell@2
  ...
  inputs:
    targetType: 'inline'
    script: |
      pwsh AzraTasks.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- task: DotNetCoreCLI@2
  displayName: 'Run UI Tests'
  inputs:
    command: 'test'
    projects: 'AzraTasks.UITests/AzraTasks.UITests.csproj'
  env:
